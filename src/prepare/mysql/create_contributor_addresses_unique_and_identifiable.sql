-- Finally worked after truncating contributor_addresses, and FAST!!!
-- NOTE: HERE WE EXPECT THE FOLLOWING:
-- contributor_addresses is the truncated version of its former self ;)
...
SET SESSION max_heap_table_size=30000000000;
SET SESSION tmp_table_size=30000000000;
...
CREATE TABLE contributor_addresses_identifiable 
    LIKE contributor_addresses;
ALTER TABLE  contributor_addresses_identifiable
       DROP KEY TRAN_INDEX;
CREATE UNIQUE INDEX TRAN_INDEX 
    ON contributor_addresses_identifiable (TRAN_ID,CMTE_ID);
...
-- Use IGNORE so that only one record with each id pair is inserted.
SELECT "Populating contributor_addresses_identifiable" AS "";
INSERT IGNORE INTO contributor_addresses_identifiable
    SELECT t1.* FROM contributor_addresses t1
        JOIN identifiables
        USING (TRAN_ID,CMTE_ID)
        GROUP BY TRAN_ID,CMTE_ID,CONTRIBUTOR_ZIP,CONTRIBUTOR_STATE,CONTRIBUTOR_CITY,CONTRIBUTOR_STREET_1;
