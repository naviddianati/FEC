...
def load_records(param_state = "newyork", all_fields = [], whereclause = ' ', record_limit = (0,5000000), do_sort = True):
...
if __name__ == "__main__":
    ...
    for counter_name, name in enumerate(set_names):
        ...
        try:
            ...
            if identity_main not in dict_identities:
                ...
                set_identities_with_given_name = set([dict_ids_full[r_id] for r_id in list_ids])
    ...
    for identity_main, list_neighboring_identities in dict_identities.iteritems():
        ...
        list_record_ids = [ r_id for identity in list_neighboring_identities for  r_id in dict_identities_full[identity] ]

        id_random_nonfocus = random.choice(list(set(list_record_ids).difference(set(dict_identities_full[identity_main])))) 
        id_random_focus = random.choice(dict_identities_full[identity_main])
        ...
        # Other values mean non error introduced.
        ...
        if len(dict_identities_full[identity_main]) < 3: error_type = None
        
        # Mark all records belonging to the focus cluster
        for r_id in dict_identities_full[identity_main]:
        ...
        for identity in list_neighboring_identities:
            ...
            list_current_ids = sorted(dict_identities_full[identity], key = lambda r_id: dict_of_records[r_id]['TRANSACTION_DT'])  
            if error_type == 2:
