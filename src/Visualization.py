...
def load_affiliation_graph(data_path, state, affiliation, component="full"):
    '''
    Load the affiliation graph generated by AffiliationAnalyzer.
    Load either the full graph or only the giant component.
    '''
    if component == "full":
        G = igraph.Graph.Load(f=data_path + str(state) + '-' + affiliation + '_graph.gml', format='gml')
    elif component == "giant":
        G = igraph.Graph.Load(f=data_path + str(state) + '-' + affiliation + '_graph_giant_component.gml', format='gml')
    return G



def plot_significance_vs_weight(G):
    '''
    Plot edge significance vs edge weight
    '''
    plt.plot(G.es['weight'], G.es['confidence'], 'o', color="#555555", alpha=0.5)
    plt.xlabel('Weight')
    plt.ylabel('P-value')
    plt.show()

def plot_edge_significance(G):
    '''
    Plot the sorted edge significance
    '''
...
#         tmp = palette._get(color)
        ...
        e['color-tuple'] = (color[0], color[1], color[2], alpha)
...
def set_vertexColor(G, palette, field='size', cluster=False):
    ...
    if cluster:
        ...
        for c in communities:
            ...
            tmp = palette._get(color)
            for v in vs:
                v['color-tuple'] = (tmp[0], tmp[1], tmp[2], alpha)
                v['color'] = rgba_to_color_name(v['color-tuple'])
...
def plot_map(center=(30, -100), bb=(-130, 25, -60, 50)):
    ...
    map.fillcontinents(color='black  ', lake_color='#a2c5ff')
...
def plot_network_on_map(G,settings):
    ...
    pal = mplPalette(numcolors, name='hot', reverse=True)
...
#     pal = mplPalette(numcolors, name='binary')
    ...
    set_edgeColor(G, palette=pal, field='confidence')
    
    
    dict_coordinates = {} 
     
    # load zip_code coordinates file
    geodata = pd.read_csv('../data/census-zipcodes.csv', sep='\t', dtype=str)
    print geodata.columns

    for index, row in geodata.iterrows():
        zipcode = row['GEOID']
#         print zipcode
        dict_coordinates[zipcode] = (float(row['INTPTLAT']), float(row[u'INTPTLONG']))
        
        

    # plot a point for each vertex in graph
    for v in G.vs:
        zipcode = v['label']
        try:
            coordinates = dict_coordinates[zipcode]
        except KeyError:
            continue
        v['y'], v['x'] = coordinates
        
    xs_tmp = [v['x'] for v in G.vs]
    ys_tmp = [v['y'] for v in G.vs]
    sizes_tmp = [v['size'] for v in G.vs]
    
    xs = [xs_tmp[i] for i in range(len(xs_tmp)) if xs_tmp[i] and ys_tmp[i]]
    ys = [ys_tmp[i] for i in range(len(xs_tmp)) if xs_tmp[i] and ys_tmp[i]]
    sizes = [sizes_tmp[i] for i in range(len(xs_tmp)) if xs_tmp[i] and ys_tmp[i]]
    
    print xs
    print (min(xs), min(ys), max(xs), max(ys))
    map = plot_map(bb=(min(xs), min(ys), max(xs), max(ys)))
    map.scatter(xs, ys, s=sizes, c=G.vs['color-tuple'] , latlon=True, zorder=100)
    
    
    
    for e in G.es:
        id0, id1 = e.source, e.target
        v0, v1 = G.vs[id0], G.vs[id1]
        x0, y0 = v0['x'], v0['y']
        x1, y1 = v1['x'], v1['y']
        print x0, x1, y0, y1
        if x0 and x1 and y0 and y1:
            color = e['color-tuple'] 
            map.plot([x0, x1, x0], [y0, y1, y0], '-', linewidth=e['width'], color=e['color-tuple'] , latlon=True)
...
if __name__ == "__main__":
    ...
    #                     affiliation='employer',
                        ...
                        e_field='confidence',
                        filter_components=False,
                        output_label="attorney-2-15pct-confidence-new"
                         )
    ...
    #                     restrict_to_neighborhood = {'label':'ATTORNEY','distance':2},
                        ...
                        e_field='confidence',
                        filter_components=False,
                        output_label="-15pct-confidence"
                        )
    
    settings = settings2
    
    G = load_affiliation_graph(settings.data_path, settings.state, settings.affiliation, component="giant")
    
    
    plot_network(G, settings)
    # plot_network_on_map(G,settings)
